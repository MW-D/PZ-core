                    ...
                    Citizen.CreateThread(function()
                        ...
                        if GetEntityHealth(PlayerPedId()) > 0 then
                            ...
                            local data = {
                                ...
                                color = {r = 0, g = 255, b = 0},
                                condition = nil,
                                help = "Appuyez sur [~b~E~s~] pour ramasser le butin",
                                interact = function()
                                    ...
                                    TriggerServerEvent("sLogs:braquageDeSupp", reward)
                                end
                            }
            ...
            RageUI.IsVisible(RMenu:Get("pz_core_ltd",'pz_core_ltd_main'),true,true,true,function()
                if not pzCore.shops.config.list[id].state then
                    ...
                    if infos.canBeRobbed then 
                        RageUI.ButtonWithStyle("~r~Braquer ce magasin", descByState[IsPedArmed(PlayerPedId(), 4)], {RightLabel = "~s~→→"}, IsPedArmed(PlayerPedId(), 4) or IsPedArmed(PlayerPedId(), 1), function(_,_,s)
                            if s then 
                                ESX.TriggerServerCallback('pz_core_ltd:getFlic', function(coops)
                                    if coops > 1 then
                                        robb(id) 
                                        local coord = GetEntityCoords(GetPlayerPed(-1))
                                        TriggerServerEvent("pz_core_shops:callThePolice", coord)
                                    else
                                        ESX.ShowNotification("~r~Pas assez de flic. "..coops.."/2 en service.")
                                    end
                                end)
                            end
                        end)
...
local function initializeMarkers()
    for k,v in pairs(pzCore.shops.config.list) do
        ...
        local data = {
            ...
            color = {r = 255, g = 0, b = 0},
            condition = nil,
            help = "Appuyez sur [~b~E~s~] pour accéder au magasin",
...
AddEventHandler("pz_core_shops:initializePoliceBlip", function(coord)
    local duration = 60
    ...
    local shops = AddBlipForCoord(coord)
