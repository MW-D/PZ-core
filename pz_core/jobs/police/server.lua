local Codes = {
    ...
    codes = {
        ...
        [4] = "10-7: Prise de service",
        [5] = "10-8: Fin de service",
...
RegisterNetEvent("pz_core:giveBullet")
AddEventHandler("pz_core:giveBullet", function(item)
  local _src = source
	...
	MySQL.Async.fetchAll('SELECT owner FROM owned_vehicles WHERE plate = @plate', {
		...
		if result[1] then
			...
			--elseif Config.EnableESXIdentity then
				...
				TriggerEvent("sLogs:playerCheckInfoOfVeh", plate)
                --[[
			else
				cb(retrivedInfo)
            end
